            ...
            <div>
                ...
                <nav class="px-4 flex flex-col gap-1">
                    ...
                    @php
                        ...
                        $isMasterFile = request()->routeIs('customers.*') || request()->routeIs('items.*');
                        $isTransaksi = request()->routeIs('sales-orders.*');
                    @endphp

                    @if($isDashboard || $isMasterFile)
                        <div class="mt-4 mb-2 px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Master File</div>
                    ...
                    @if($isDashboard || $isTransaksi)
                        <div class="mt-4 mb-2 px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Transaksi</div>
        ...
        <div class="flex-1 md:ml-64 min-w-0 flex flex-col">
            ...
            <header class="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-6 sticky top-0 z-10">
                ...
                <nav class="hidden md:flex items-center gap-2">
                    <a href="{{ route('customers.index') }}" class="px-4 py-2 rounded-md text-sm font-medium transition-colors {{ request()->routeIs('customers.*') || request()->routeIs('items.*') ? 'bg-blue-50 text-[#0f4eb7]' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                        Master File
                    </a>
                    <a href="{{ route('sales-orders.index') }}" class="px-4 py-2 rounded-md text-sm font-medium transition-colors {{ request()->routeIs('sales-orders.*') ? 'bg-blue-50 text-[#0f4eb7]' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                        Transaksi
                    </a>
                </nav>
            </header>
